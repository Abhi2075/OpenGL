...
void initilize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				"u_Diffuse_Light = u_LD * u_KD * max(dot(light_source,t_norm),0.0);" \
			"}" \
			"gl_Position = u_perspective_projection_matrix * u_model_view_matrix * vPosition;" \
		"}";
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"{" \
			...
			"if(u_LKeyPress == 1)" \
			"{" \
				"color = vec4(u_Diffuse_Light,1.0);" \
			"}" \
			"else" \
			"{" \
