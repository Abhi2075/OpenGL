...
GLubyte checklmage[checklmageHeight][checklmageWidth][4];
GLuint texName;
...
void initilize(void)
{
	...
	//AS//const GLfloat rectangleVertices[] =
	//AS//{ 
	//AS//	//Front
	//AS//	CubeVertex,CubeVertex,Zero,
	//AS//	-CubeVertex,CubeVertex,Zero,
	//AS//	-CubeVertex,-CubeVertex,Zero,
	//AS//	CubeVertex,-CubeVertex,Zero,
	//AS//};
...
void loadGlTexture(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, checklmageWidth, checklmageHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, checklmage);
...
void makeChecklmage(void)
{
	...
	for (i = 0; i < checklmageHeight; i++)
	{
		for (j = 0; j < checklmageWidth; j++)
		{
			c = (((i & 0x8) == 0) ^ ((j & 0x8) == 0)) * 255;
			checklmage[i][j][0] = (GLubyte)c;
			checklmage[i][j][1] = (GLubyte)c;
			checklmage[i][j][2] = (GLubyte)c;
			checklmage[i][j][3] = (GLubyte)255;
		}
	}
}
...
void display(void)
{
	...
	GLfloat CubeVertex_Zero = 1.0f;
	GLfloat CubeVertex_One = 1.41421;
	GLfloat CubeVertex_Two = 2.41421;
	GLfloat CubeVertex_Three = 2.;
	GLfloat Zero = 0.0f;

	const GLfloat rectangleVertices_Front[] =
	{ 
		//Front
		-CubeVertex_Three,-CubeVertex_Zero,Zero,
		-CubeVertex_Three,CubeVertex_Zero,Zero,
		Zero,CubeVertex_Zero,Zero,
		Zero,-CubeVertex_Zero,Zero,
	};

	const GLfloat rectangleVertices_Anguler[] =
	{
		//Front
		CubeVertex_Zero,-CubeVertex_Zero,Zero,
		CubeVertex_Zero,CubeVertex_Zero,Zero,
		CubeVertex_Two,CubeVertex_Zero,-CubeVertex_One,
		CubeVertex_Two,-CubeVertex_Zero,-CubeVertex_One,
	};
	...
	glBindBuffer(GL_ARRAY_BUFFER, gVbo_Position_Cube);
	glBufferData(GL_ARRAY_BUFFER, 4 * 3 * sizeof(GLfloat), rectangleVertices_Front, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(ADS_ATTRUBUTE_VERTEX_Position, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(ADS_ATTRUBUTE_VERTEX_Position);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	...
	glBindBuffer(GL_ARRAY_BUFFER, gVbo_Position_Cube);
	glBufferData(GL_ARRAY_BUFFER, 4 * 3 * sizeof(GLfloat), rectangleVertices_Anguler, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(ADS_ATTRUBUTE_VERTEX_Position, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(ADS_ATTRUBUTE_VERTEX_Position);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
