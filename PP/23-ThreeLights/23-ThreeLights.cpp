...
void initilize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"{" \
			...
			"mat3 normal_matrix = mat3(u_view_matrix * u_model_matrix);" \
			...
			"}" \
			"u_Phong_ADS_Light = u_Phong_ADS_Light_temp[0] + u_Phong_ADS_Light_temp[1] + u_Phong_ADS_Light_temp[2];" \
		"}" \
		"else" \
		"{" \
	...
	if (iShaderCompiledStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(GpFile, "Vertex Shader1 compilatin Log : %s\n", szInfoLog);
				free(szInfoLog);
				unInitilize();
				exit(0);
			}
	...
	if (iShaderCompiledStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(GpFile, "Fragment Shader1 compilatin Log : %s\n", szInfoLog);
				free(szInfoLog);
				unInitilize();
				exit(0);
			}
	...
	viewMatrixUniform = glGetUniformLocation(gShaderProgramObject_PV, "u_view_matrix");
	...
	/*for (int i = 0; i < 3; i++)
	{
		LaUniform[i] = glGetUniformLocation(gShaderProgramObject_PV, "u_LA[i]");
		LdUniform[i] = glGetUniformLocation(gShaderProgramObject_PV, "u_LD[i]");
		LsUniform[i] = glGetUniformLocation(gShaderProgramObject_PV, "u_LS[i]");
		lightPositionUniform[i] = glGetUniformLocation(gShaderProgramObject_PV, "u_Light_Position[i]");
	}*/

	LaUniform[0] = glGetUniformLocation(gShaderProgramObject_PV, "u_LA[0]");
	LdUniform[0] = glGetUniformLocation(gShaderProgramObject_PV, "u_LD[0]");
	LsUniform[0] = glGetUniformLocation(gShaderProgramObject_PV, "u_LS[0]");
	lightPositionUniform[0] = glGetUniformLocation(gShaderProgramObject_PV, "u_Light_Position[0]");

	LaUniform[1] = glGetUniformLocation(gShaderProgramObject_PV, "u_LA[1]");
	LdUniform[1] = glGetUniformLocation(gShaderProgramObject_PV, "u_LD[1]");
	LsUniform[1] = glGetUniformLocation(gShaderProgramObject_PV, "u_LS[1]");
	lightPositionUniform[1] = glGetUniformLocation(gShaderProgramObject_PV, "u_Light_Position[1]");

	LaUniform[2] = glGetUniformLocation(gShaderProgramObject_PV, "u_LA[2]");
	LdUniform[2] = glGetUniformLocation(gShaderProgramObject_PV, "u_LD[2]");
	LsUniform[2] = glGetUniformLocation(gShaderProgramObject_PV, "u_LS[2]");
	lightPositionUniform[2] = glGetUniformLocation(gShaderProgramObject_PV, "u_Light_Position[2]");
	...
	//*************************************** Per Vertex Lighting End ***************************************************************
	 ...
	 if (iShaderCompiledStatus == GL_FALSE)
	 {
	 	...
	 	if (iInfoLogLength > 0)
	 	{
	 		...
	 		if (szInfoLog != NULL)
	 		{
	 			...
	 			fprintf(GpFile, "Vertex Shader2 compilatin Log : %s\n", szInfoLog);
	 			free(szInfoLog);
	 			unInitilize();
	 			exit(0);
	 		}
	 ...
	 if (iShaderCompiledStatus == GL_FALSE)
	 {
	 	...
	 	if (iInfoLogLength > 0)
	 	{
	 		...
	 		if (szInfoLog != NULL)
	 		{
	 			...
	 			fprintf(GpFile, "Fragment Shader2 compilatin Log : %s\n", szInfoLog);
	 			free(szInfoLog);
	 			unInitilize();
	 			exit(0);
	 		}
	 ...
	 viewMatrixUniform = glGetUniformLocation(gShaderProgramObject_PF, "u_view_matrix");
	...
	/* for (int i = 0; i < 3; i++)
	 {
	 	LaUniform[i] = glGetUniformLocation(gShaderProgramObject_PF, "u_LA[i]");
	 	LdUniform[i] = glGetUniformLocation(gShaderProgramObject_PF, "u_LD[i]");
	 	LsUniform[i] = glGetUniformLocation(gShaderProgramObject_PF, "u_LS[i]");
	 	lightPositionUniform[i] = glGetUniformLocation(gShaderProgramObject_PF, "u_Light_Position[i]");
	 }*/

	 LaUniform[0] = glGetUniformLocation(gShaderProgramObject_PF, "u_LA[0]");
	 LdUniform[0] = glGetUniformLocation(gShaderProgramObject_PF, "u_LD[0]");
	 LsUniform[0] = glGetUniformLocation(gShaderProgramObject_PF, "u_LS[0]");
	 lightPositionUniform[0] = glGetUniformLocation(gShaderProgramObject_PF, "u_Light_Position[0]");

	 LaUniform[1] = glGetUniformLocation(gShaderProgramObject_PF, "u_LA[1]");
	 LdUniform[1] = glGetUniformLocation(gShaderProgramObject_PF, "u_LD[1]");
	 LsUniform[1] = glGetUniformLocation(gShaderProgramObject_PF, "u_LS[1]");
	 lightPositionUniform[1] = glGetUniformLocation(gShaderProgramObject_PF, "u_Light_Position[1]");

	 LaUniform[2] = glGetUniformLocation(gShaderProgramObject_PF, "u_LA[2]");
	 LdUniform[2] = glGetUniformLocation(gShaderProgramObject_PF, "u_LD[2]");
	 LsUniform[2] = glGetUniformLocation(gShaderProgramObject_PF, "u_LS[2]");
	 lightPositionUniform[2] = glGetUniformLocation(gShaderProgramObject_PF, "u_Light_Position[2]");
...
void display(void)
{
	...
	else if (bFragment_Light == true)
	{
		...
		if (bLight == true)
		{
			...
			light[0].LightPosition = { 0.0f, radius * (GLfloat)sin(Aangle1), radius * (GLfloat)cos(Aangle1), 1.0f };

			light[1].LightAmbintColor = { 0.0f, 0.0f, 0.0f };
			light[1].LightDiffuseColor = { 0.0f, 1.0f, 0.0f };
			light[1].LightSpecularColor = { 0.0f, 1.0f, 0.0f };
			light[1].LightPosition = { radius * (GLfloat)sin(Aangle2), 0.0f, radius * (GLfloat)cos(Aangle2), 1.0f };

			light[2].LightAmbintColor = { 0.0f, 0.0f, 0.0f };
			light[2].LightDiffuseColor = { 0.0f, 0.0f, 1.0f };
			light[2].LightSpecularColor = { 0.0f, 0.0f, 1.0f };
			light[2].LightPosition = { radius * (GLfloat)sin(Aangle3), radius * (GLfloat)cos(Aangle3), 0.0f, 1.0f };
			...
			glUniform3fv(LaUniform[0], 1, light[0].LightAmbintColor);
			glUniform3fv(LdUniform[0], 1, light[0].LightDiffuseColor);
			glUniform3fv(LsUniform[0], 1, light[0].LightSpecularColor);
			glUniform4fv(lightPositionUniform[0], 1, light[0].LightPosition);

			glUniform3fv(LaUniform[1], 1, light[1].LightAmbintColor);
			glUniform3fv(LdUniform[1], 1, light[1].LightDiffuseColor);
			glUniform3fv(LsUniform[1], 1, light[1].LightSpecularColor);
			glUniform4fv(lightPositionUniform[1], 1, light[1].LightPosition);

			glUniform3fv(LaUniform[2], 1, light[2].LightAmbintColor);
			glUniform3fv(LdUniform[2], 1, light[2].LightDiffuseColor);
			glUniform3fv(LsUniform[2], 1, light[2].LightSpecularColor);
			glUniform4fv(lightPositionUniform[2], 1, light[2].LightPosition);
		...
		modelMatrix = vmath::translate(0.0f, 0.0f, -3.0f) * vmath::rotate(Sangle, 1.0f, 0.0f, 0.0f);
		modelMatrix = modelMatrix * vmath::rotate(Sangle, 0.0f, 1.0f, 0.0f);
		modelMatrix = modelMatrix * vmath::rotate(Sangle, 0.0f, 0.0f, 1.0f);
	...
	if (bAnimate == true)
	{
		Sangle = UpdateAngle(Sangle);
	}
	if (bLightAnimate == true)
	{
		Aangle1 = UpdateAngle(Aangle1);
		Aangle2 = UpdateAngle(Aangle2);
		Aangle3 = UpdateAngle(Aangle3);
	}
